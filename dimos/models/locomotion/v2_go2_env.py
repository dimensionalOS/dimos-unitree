...
class V2Go2Env:
    def __init__(self, num_envs, env_cfg, obs_cfg, reward_cfg, command_cfg, show_viewer=False):
        ...
        # # pick a random terrain each reset
        # terrain_type = random.choice(terrain_types[:4])  # flat, uniform, pyramid, obstacles
        ...
        terrains.pos = (-7.5, -7.5, 0.5) # 1.0 = 5 
    ...
    def _resample_commands(self, envs_idx):
        self.commands[envs_idx, 0] = (self.command_cfg["lin_vel_x_range"][1] - self.command_cfg["lin_vel_x_range"][0]) * \
                                     torch.rand(len(envs_idx), device=gs.device) + self.command_cfg["lin_vel_x_range"][0]
        self.commands[envs_idx, 1] = (self.command_cfg["lin_vel_y_range"][1] - self.command_cfg["lin_vel_y_range"][0]) * \
                                     torch.rand(len(envs_idx), device=gs.device) + self.command_cfg["lin_vel_y_range"][0]
        self.commands[envs_idx, 2] = (self.command_cfg["ang_vel_range"][1] - self.command_cfg["ang_vel_range"][0]) * \
                                     torch.rand(len(envs_idx), device=gs.device) + self.command_cfg["ang_vel_range"][0]
